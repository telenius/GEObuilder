...
# COMBINING the fastq parameter file to other parameters (metadata) is made using the above guidelines, within joinParameterFiles subroutine in runnerHelpersForAllVersions in PIPERUNNERS folder
    ...
    if [ "${TEMPcount2}" -ne "${TEMPcount}" ] ; then echo "Some FASTQ read1 path(s) / file name(s) are EMPTY - correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi
    
    if [ ${weHaveRead2} -ne 0 ]; then
    TEMPcount2=$(($( cat TEMP_fastqRead2.txt | grep -c "" )))    
    if [ "${TEMPcount2}" -ne "${TEMPcount}" ] ; then echo "Some FASTQ read2 path(s) / file name(s) are EMPTY (or you are trying to run paired and single end samples in same run - that is not allowed) Correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi    
    fi
    
    # Checking that the folder names are not empty ..
    if [ -s TEMP_fastqFolders.txt ]; then
    TEMPcount2=$(($( cat TEMP_fastqFolders.txt | grep -c "" )))
    if [ "${TEMPcount2}" -ne "${TEMPcount}" ] ; then echo "Some FASTQ folder path(s) are EMPTY (or you are trying to run paired and single end samples in same pipeline run - that is not allowed) Correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi    
    fi
    ...
    if [ "${TEMPcount2}" -ne "${TEMPcount}" ] ; then
        ...
        echo "    Correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err
        fastqDataOK=0
    fi   
    ...
    if [ "${TEMPcount2}" -ne "${TEMPcount}" ] ; then echo "Some FASTQ read1 path(s) are NOT UNIQUE for sample ${TEMP_sample} (some lanes are given twice) - correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi
    ...
    do
        if [ ! -d ${file} ] ; then echo "FASTQ path ${file} does not exist (or is not a directory). Correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi    
    done    
    ...
    for file in ${TEMP_list}
    do
        if [ ! -s ${file} ] ; then echo "FASTQ read1 file ${file} does not exist (or is empty file). Correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi    
    done
    ...
    for file in ${TEMP_list}
    do
        if [ ! -s ${file} ] ; then echo "FASTQ read2 file ${file} does not exist (or is empty file). Correct your GEO_fastqPaths.txt file !" >> ../FASTQ_LOAD.err ;fastqDataOK=0; fi    
    done
    fi    
